...
namespace ServiceLayer.Services
{
    public class MovieService : IMovieService
    {
        ...
        private const string BucketName = "moviemate-videos";
        ...
        public async Task Delete(int id)
        {
            ...
            Movie movie = await _repo.Get(id);
        ...
        public async Task<string> Upload(IFormFile file)
        {
            ...
            using (var stream = new FileStream(@"C:\Users\user\Desktop\Holberton-Final-Project\Server Side\MovieMateApp\App\moviemate-426618-ee469cb55b6d.json", FileMode.Open))
            {
        ...
        public async Task<MoviePointDto> Like(MovieRaitingDto movieRaitingDto)
        {
            ...
            {
                if (existMovieRaiting.DisLikeActive)
                {
                    movie.LikeCount = movie.LikeCount + 1;
                    movie.DisLikeCount = movie.DisLikeCount - 1;
                    movie.Raiting = movie.Raiting + 0.2f;
                }
        ...
        public async Task<MoviePointDto> DisLike(MovieRaitingDto movieRaitingDto)
        {
            ...
            {
                if (existMovieRaiting.LikeActive)
                {
                    movie.DisLikeCount = movie.DisLikeCount + 1;
                    movie.LikeCount = movie.LikeCount - 1;
                    movie.Raiting = movie.Raiting - 0.2f;
        ...
        {
            var movies = await _repo.GetAll();
            var randomMovies = movies.OrderBy(x => Guid.NewGuid()).Take(10).ToList();
            return randomMovies;
        }
    }
}
